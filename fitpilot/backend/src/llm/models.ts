...
/**
 * Gemini 3 models are reasoning models tuned for their default sampling
 * settings — Google explicitly recommends against overriding temperature /
 * top_p / top_k for general use, so we only set one when a caller has a
 * specific reason to (e.g. more deterministic JSON extraction).
 */
export function makeModel(tier: Tier, temperature?: number): ChatGoogleGenerativeAI {
