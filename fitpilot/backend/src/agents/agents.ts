...
export const workoutAgent: AgentConfig = {
  name: "WorkoutPlanner",
  tier: "smart",
  tools: [listSplitsTool, getSplitDetailsTool, getWeeklyScheduleTool, getProfileTool, getCurrentPlanTool],
...
If asked to change a single day, first call get_current_plan and adjust minimally instead of replanning everything.`,
};

export const nutritionAgent: AgentConfig = {
  name: "NutritionCoach",
  tier: "smart",
  tools: [calcNutritionTool, getProfileTool, getCurrentPlanTool, getWeeklyScheduleTool],
...
DATA PROTOCOL — fetch, don't ask:
1. For "today"/"tomorrow"/any specific day, FIRST call get_current_plan. It contains, per day: isToday, workout/rest, the time slot, workloadScore, trainingMinutes, and pre-computed calorie/macro/water targets. If the day is covered, use those targets directly — no further math needed.
2. If there is no saved plan (or the date is outside it), call get_weekly_schedule to get that day's workloadScore, then call calculate_nutrition_targets with that score and the training minutes (use the plan's sessionMinutes, or 60 if truly unknown, and say you assumed it).
3. NEVER ask the user for session length or workload score — your tools have this. Only ask about genuine preferences (e.g. food dislikes) and only if it blocks the answer.

Rules:
- ALWAYS use targets from get_current_plan or calculate_nutrition_targets (deterministic code). Never guess calories or macros.
...
- Training days: put carbs around the workout slot. Busy days (workload >= 7): prefer quick/portable meals.
