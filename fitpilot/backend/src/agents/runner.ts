...
export async function runAgent(
  ...
  for (let i = 0; i < maxIter; i++) {
    ...
    const ai = (await withKeyRotation(async () => {
      // cfg.temperature is left undefined by default so Gemini 3's tuned
      // sampling defaults apply; agents only set it for a specific reason.
      const model = makeModel(cfg.tier, cfg.temperature);
