...
export const getCurrentPlanTool = tool(
  async () => {
    ...
    if (!plan) return JSON.stringify({ plan: null, note: "No weekly plan generated yet." });
    const days = Object.entries(plan.days).map(([date, d]: [string, any]) => ({
      date,
      type: d.type,
      dayName: d.dayName ?? "Rest",
      slot: d.slot ?? null,
      calories: d.nutrition?.calories ?? null,
    }));
    return JSON.stringify({
  ...
  },
  {
    ...
    description: "Returns a summary of the currently saved weekly plan (split, training days, calories).",
    schema: z.object({}),
  }
);
