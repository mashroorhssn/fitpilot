...
export async function sendNightlyDigest(): Promise<{ sent: boolean; reason?: string; preview?: string; date?: string }> {
  ...
  if (!ENV.SMTP_USER || !ENV.SMTP_PASS || !ENV.EMAIL_TO) {
    return { sent: false, reason: "SMTP not configured (SMTP_USER/SMTP_PASS/EMAIL_TO).", preview: digest.html, date: digest.date };
  }
  ...
  return { sent: true, preview: digest.html, date: digest.date };
}
